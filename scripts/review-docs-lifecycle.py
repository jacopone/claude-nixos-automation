...
def main():
    ...
    if draft_flagged:
        ...
        for doc in draft_flagged:
            ...
            print(f"     {BLUE}Recommendation:{RESET} Archive or update status to 'active'\n")
    else:
        print(f"{GREEN}✓ No draft documents older than {DRAFT_THRESHOLD} days{RESET}\n")

    if ephemeral_flagged:
        print(f"{YELLOW}🗄️  Ephemeral documents older than {EPHEMERAL_THRESHOLD} days:{RESET}\n")
        for doc in ephemeral_flagged:
            ...
            if '.claude/sessions' in str(doc['path']):
                ...
                print(f"     {BLUE}Recommendation:{RESET} Already in sessions, verify archive location (.claude/sessions/archive/{archive_date}/)\n")
            else:
                print(f"     {BLUE}Recommendation:{RESET} Move to .claude/sessions/archive/{doc['created'][:7]}/\n")
    else:
        print(f"{GREEN}✓ No ephemeral documents older than {EPHEMERAL_THRESHOLD} days{RESET}\n")
    ...
    print(f"  Ephemeral docs flagged (>{EPHEMERAL_THRESHOLD} days): {len(ephemeral_flagged)}")
    print(f"{'='*70}\n")

...
def test_phase4_permission_patterns():
    ...
    try:
        ...
        try:
            ...
            print(
                f"{GREEN}✓{RESET} Generated optimization suggestions: {len(suggestions)}"
            )
...
def test_phase5_context_optimizer():
    ...
    try:
        ...
        print(
            f"{GREEN}✓{RESET} ContextUsageTracker and ContextOptimizer imported successfully"
        )
        ...
        try:
            ...
            print(
                f"{GREEN}✓{RESET} Analyzed section usage: {len(usage_stats)} sections"
            )

            # Test noise detection
            noise_sections = optimizer.detect_noise_sections(
                min_loads=1, utilization_threshold=0.5
            )
